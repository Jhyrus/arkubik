...
<script>
  function setMapContactHeight(){
    var nodes = document.getElementsByClassName("mapa-contacto");
    console.log(nodes[0]);
    nodes[0].childNodes[1].style.height = nodes[0].childNodes[1].offsetWidth+'px';
    nodes[0].childNodes[1].childNodes[1].childNodes[1].style.height = nodes[0].childNodes[1].offsetWidth+'px';
    $('.mapa-contacto').click(function () {
      window.open('https://www.google.com/maps?ll=-16.540358,-68.077447&z=17&t=m&hl=es-419&gl=ES&mapclient=embed&q=Torre+Lydia+Calle+21+La+Paz+Bolivia','_blank');
    });
  }
  function hoverProducts(text,img,textinitial,texthover){
    var nodespan = document.getElementsByClassName(text);
    console.log(nodespan[0]);
    var node = document.getElementsByClassName(img);
    nodespan = nodespan[0].childNodes[1].childNodes[1].childNodes[0];

    node[0].addEventListener("mouseover", function( event ) {   
      nodespan.innerHTML += "<span style='font-family:Raleway; line-height:4px; display:block;'>"+texthover+"</span>";
    }, false);

    node[0].addEventListener("mouseout", function( event ) {   
      nodespan.innerHTML = "<strong style='font-family:Raleway;'>"+textinitial+"</strong>";
    }, false);
  }
  window.onload = setMapContactHeight;


  document.addEventListener('DOMContentLoaded', function() {
   // your code here
    window.onload = hoverProducts("disenho-y-vaciado-text","disenho-y-vaciado-img","Diseño y Vaciado"," <br/>&nbsp;de Pavimentos de Hormigón");
    window.onload = hoverProducts("preparado-de-superficies-text","preparado-de-superficies-img","Preparado de Superficies","<br/>&nbsp;de Concreto y Revestimiento");
    window.onload = hoverProducts("pulido-de-pavimentos-text","pulido-de-pavimentos-img","Pulido de Pavimentos","<br/>&nbsp;de Concreto");
    window.onload = hoverProducts("recuperacion-text","recuperacion-img","Recuperación","<br/>&nbsp;de Losas de Concreto");
  }, false);
